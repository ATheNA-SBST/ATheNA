...
switch argv.local_minimization_algo
    case 'RED'
        disp('Robustness Ellipsoid Descent')
...
c_sol       = min(periter.cost);

if opt.dispinfo
    fprintf(['\nLocal descent used up ',num2str(RUNSTATS.nb_function_evals_this_run() - temp ),' trajectories.\n']);
    disp(['Post-descent cost = ',num2str(c_sol)])
end

outargv = struct('h_sol', h_sol, 'nboptiter', nboptiter, 'c_sol', c_sol, 'end_opt', end_opt);
