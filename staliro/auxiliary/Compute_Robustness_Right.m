...
% end
otherData = struct('timeStats',[],'t',[],'U',[],'y',[]);
...
%% Plot falsifying sample

global BEE_param

if cost <= 0 && BEE_param.Nfig >= 1
    
    cp_temp = [0, temp_ControlPoints];
    cp_array = diff(cp_temp);

    t = T;
    u = zeros(length(t),length(cp_array));
    y = YT;

    % Plot final input
    figure(BEE_param.Nfig+10)
    clf
    for ii = 1:length(cp_array)
        n = linspace(0,staliro_SimulationTime,cp_array(ii));
        u(:,ii) = feval(staliro_opt.interpolationtype{ii},n,UPoint(cp_temp(ii)+1:cp_temp(ii+1)),t);
        subplot(length(cp_array),1,ii)
        grid on
        %hold on
        plot(t,u(:,ii),'b')
        plot(n,UPoint(cp_temp(ii)+1:cp_temp(ii+1)),'b*')
        ylim(staliro_InputBounds(ii,:))
        xlabel('$Time~[s]$','Interpreter','latex','FontSize',16)
        ylabel(BEE_param.InStr{ii},'Interpreter','latex','FontSize',16)
    end
    sgtitle('$Input~-~Falsifying~AT1$','Interpreter','latex','FontSize',20)

    % Plot final output
    figure(BEE_param.Nfig+11)
    clf
    for ii = 1:size(y,2)
        ...
        %hold on
        plot(t,y(:,ii),'g')
        ylim(BEE_param.OutRange(ii,:))
        xlabel('$Time~[s]$','Interpreter','latex','FontSize',16)
        ylabel(BEE_param.OutStr{ii},'Interpreter','latex','FontSize',16)
    end
    sgtitle('$Output~-~Falsifying~AT1$','Interpreter','latex','FontSize',20)
    
%     % Plot car distance
%     figure(BEE_param.Nfig+12)
%     clf
%     for ii = 1:size(y,2)-1
%         subplot(size(y,2)-1,1,ii)
%         hold on
%         grid on
%         plot(t,y(:,ii+1)-y(:,ii),'r')
%         if ii == size(y,2)-1
%             plot([0, staliro_SimulationTime],[40, 40],'k--')
%         end
%         ylim([0, 50])
%         xlabel('$Time~[s]$','Interpreter','latex','FontSize',16)
%         ylabel(sprintf('$y_{%i}-y_{%i}$',ii+1,ii),'Interpreter','latex','FontSize',16)
%     end
%     sgtitle('$Car~distance~-~Falsifying~CC1$','Interpreter','latex','Fontsize',20)
