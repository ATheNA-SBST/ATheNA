...
function [cost, ind, rob, fitness, cur_par, otherData] = Compute_Robustness(input, auxData)
...
% Read-only parameters for BEE value
global BEE_param;
...
if iscell(input)
    ...
    if length(input)>1
        ...
        warning('off','Simulink:Logging:LegacyModelDataLogsFormat');
        [rob{1}, ind{1}, cur_par{1}, ~, otherData{1}] = Compute_Robustness_Right(staliro_InputModel, staliro_InputModelType , input{1}, auxData);
        [fitness{1}, ~, ~, ~, ~] = BEE_value(staliro_InputModel, input, otherData{1});
        disp('problem');
        cost{1} = rob{1}*BEE_param.coeffRob+fitness{1}*(1-BEE_param.coeffRob);
 %       cost{1} = min(rob{1},fitness{1});
 %       cost{1}=rob{1};
        warning('on','Simulink:Logging:LegacyModelDataLogsFormat');
    end
    ...
    if (staliro_opt.stochastic == 1) && (staliro_opt.n_workers > 1)
        ...
        % TODO: if the robustness is computed in parallel, change the
        % function used in globalBridge.m
        ...
        if iscell(staliro_InputModel)
            ...
            % ToDo: currently the system evaluates the output two times
            % with excessive computational time
            [rob, ind, cur_par, ~, otherData] = Compute_Robustness_Right(staliro_InputModel, staliro_InputModelType , input, auxData);
            [fitness, ~, ~, ~, ~] = BEE_value(staliro_InputModel, input, otherData);

            cost = rob*BEE_param.coeffRob+fitness*(1-BEE_param.coeffRob);
            global iternum;
            a=iternum;
            disp(['iteration',9,num2str(a),9,'fitness:',9,num2str(fitness),9,'robustness:',9,num2str(rob),9,'cost:',9,num2str(cost)]);
            iternum=a+1;
            %cost = min(rob,fitness);
            %cost=rob;
            % Store timing statistics
            if staliro_opt.TimeStatsCollect
