...
if nargout>1
    if isa(curVal{1},'hydis')
        ...
        history.rob(1:opt.n_workers) = hydisc2m(curRob)';
        history.fitness(1:opt.n_workers) = hydisc2m(curFit)';
        ...
        history.rob(1:opt.n_workers) = cell2mat(curRob)';
        history.fitness(1:opt.n_workers) = cell2mat(curFit)';
...
bestRob = curRob{minmax_idx};
bestFit = curFit{minmax_idx};
bestCost = minmax_val;

run.bestRob = curRob{minmax_idx};
run.bestFit = curFit{minmax_idx};
...
if (fcn_cmp(curRob{minmax_idx},0) && StopCond)
    if nargout>1
...
for i = 2:nSamples/opt.n_workers
    ...
    if nargout>1
        if isa(curVal{1},'hydis')
            history.rob((i-1)*opt.n_workers+1 : i*opt.n_workers) = hydisc2m(curRob)';
            history.fitness((i-1)*opt.n_workers+1 : i*opt.n_workers) = hydisc2m(curFit)';
            history.cost((i-1)*opt.n_workers+1 : i*opt.n_workers) = hydisc2m(curVal)';
        else
            history.rob((i-1)*opt.n_workers+1 : i*opt.n_workers) = cell2mat(curRob)';
            history.fitness((i-1)*opt.n_workers+1 : i*opt.n_workers) = cell2mat(curFit)';
