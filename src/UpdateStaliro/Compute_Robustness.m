...
if iscell(input)
    ...
    if length(input)>1
        ...
        [fitness{1}, ~] = AthenaManFit(staliro_InputModel, input, otherData{1});
        cost{1} = rob{1}*staliro_opt.coeffRob+fitness{1}*(1-staliro_opt.coeffRob);
        warning('on','Simulink:Logging:LegacyModelDataLogsFormat');
    end
    ...
    if (staliro_opt.stochastic == 1) && (staliro_opt.n_workers > 1)
        ...
        if iscell(staliro_InputModel)
            ...
            [fitness, ~] = AthenaManFit(staliro_InputModel, input, otherData);
            cost = rob*staliro_opt.coeffRob+fitness*(1-staliro_opt.coeffRob);
