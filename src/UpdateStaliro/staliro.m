...
if ((length(staliro_opt.interpolationtype)>1) && (length(cp_array)~=length(staliro_opt.interpolationtype))) 
    error('S-Taliro: The number of inputs are not equal to the number of interpolation types. Common error: Make sure you are using a cell array for the interpolation type.');
end
...
results.run(staliro_opt.runs) = struct('bestRob',[],'bestFit',[],'bestSample',[],'nTests',[],'bestCost',[],'paramVal',[],'falsified',[],'time',[]);
if opt.TimeStatsCollect
    history(staliro_opt.runs) = struct('rob',[],'fitness',[],'samples',[],'cost',[],'simTimes',[],'robTimes',[]);
else
    history(staliro_opt.runs) = struct('rob',[],'fitness',[],'samples',[],'cost',[]);
end
