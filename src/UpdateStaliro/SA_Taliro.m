...
if ~isequal(opt.parameterEstimation,1)
    disp([' Initial aut fitness value ==> ', num2str(curRob)]);
    disp([' Initial man fitness value ==> ', num2str(curFit)]);
    disp([' Initial Athena value      ==> ', num2str(curVal)]);
end
    ...
    if toc(tmc)<opt.optim_params.max_time
        ...
        if ~isempty(params.fRestarts) && (mod(i,params.fRestarts)==0)
            ...
            if ( params.apply_local_descent_this_run && mcAccept(curVal1,run.bestCost,betaX,betaL) ==1)
                ...
                c_orig = curVal1;   % Original Athena value
            ...
            if fcn_cmp(curVal1,bestCost) || fcn_cmp(curRob1,0)
                ...
                fprintf('Best aut fitness\t===>\t%f\n', bestRob)
                fprintf('Best man fitness\t===>\t%f\n', bestFit)
                fprintf('Best Athena fitness\t==>\t%f\n', bestCost);
                % disp(['Best Athena fitness ==>', num2str(best')]);
